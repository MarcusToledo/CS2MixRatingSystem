...
5. **`reset_all_data(p_admin_steamid text, p_admin_name text, p_reason text) RETURNS void`** — espelha `ResetAllDataWithAuditAsync` (linha 949): insere audit log com `action = 'WIPE'`, depois `DELETE FROM` em `rating_history`, `match_player_stats`, `season_ratings`, `matches`, `players`, `web_sync_queue` (nessa ordem, por causa das FKs) e `UPDATE web_sync_state SET wipe_pending = true`.
